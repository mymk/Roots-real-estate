<?php
    ...
    if (isset($_GET['post_type']) && $_GET['post_type'] == 'sale') {
        $sale = true;
    }

    if (isset($_GET['post_type']) && $_GET['post_type'] == 'rent') {
        $rental = true;
    }
    ...
    if (isset($_GET['heating_select'])) {
        $heating_select = $_GET['heating_select'];
    }
...
<div class="shadded boxed add-top">
	...
	<form role="search" method="get" class="search-form  remove-bottom" action="<?php echo home_url('/'); ?>" role="form">
		...
		<div class="form-group form-inline">
			...
			<span>
				<input id="type-sale" class="form-control" type="radio" name="post_type" data-price-step="1000" value="sale" <?php if ($sale) {
    echo 'checked';
} ?>>
				<label for="type-sale"><?php _e('Sales', 'roots-immo'); ?></label>
			</span>
			<span>
				<input id="type-rent" class="form-control" type="radio" name="post_type" data-price-step="100" value="rent" <?php if ($rental) {
    echo 'checked';
} ?>>
		...
		<div class="form-group">
			...
			<select id="type-select" name="type_select" class="custom-select form-control" title="Property type">
				...
				<?php foreach ($types_array as $key => $name) : ?>
					<option value="<?php echo $key; ?>" <?php if ($type_select == $key) {
    echo 'selected';
			...
			<select id="heating-select" name="heating_select" class="custom-select form-control" title="Heating type">
				<option value="0" <?php if ($heating_select == '0') {
    echo 'selected';
} ?>><?php _e('Heating', 'roots-immo'); ?></option>
				<option value="value1" <?php if ($heating_select == 'value1') {
    echo 'selected';
} ?>>C/C</option>
				<option value="value2" <?php if ($heating_select == 'value2') {
    echo 'selected';
} ?>>I/E</option>
				<option value="value3" <?php if ($heating_select == 'value3') {
    echo 'selected';
} ?>>I/G</option>
			</select>
		</div>
		...
		<div class="form-group">
			...
			<input id="s" name="s" type="search" value="<?php if (is_search()) {
    echo get_search_query();
} ?>" class="search-field form-control" placeholder="<?php _e('Search', 'roots-immo'); ?> <?php bloginfo('name'); ?>" autocomplete="off">
