...
<div class="shadded boxed add-top">
    ...
    <form role="search" method="get" class="search-form  remove-bottom" action="<?php echo home_url('/'); ?>"
          role="form">

        <div class="form-group form-inline">
            <label for="type-sale" class="sr-only"><?php _e('Type of property', 'roots-immo'); ?></label>
			<span>
				<input id="type-sale" class="form-control" type="radio" name="post_type" data-price-step="1000"
                       value="sale" <?php if ($sale) echo "checked"; ?>>
				<label for="type-sale"><?php _e('Sales', 'roots-immo'); ?></label>
			</span>
			<span>
				<input id="type-rent" class="form-control" type="radio" name="post_type" data-price-step="100"
                       value="rent" <?php if ($rental) echo "checked"; ?>>
        ...
        <div class="form-group">
            ...
            <select id="type-select" name="type_select" class="custom-select form-control" title="Property type">
                ...
                <?php foreach ($types_array as $key => $name) : ?>
                    <option
                        value="<?php echo $key; ?>" <?php if ($type_select == $key) echo 'selected'; ?>><?php echo $name; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="form-group advanced-search">
            ...
            <select id="heating-select" name="heating_select" class="custom-select form-control" title="Heating type">
                <option
                    value="0" <?php if ($heating_select == '0') echo 'selected'; ?>><?php _e('Heating', 'roots-immo'); ?></option>
        ...
        <div class="form-group advanced-search">
            ...
            <input id="max-price" class="form-control" name="max_price" type="number" step="100" min="0"
        ...
        <div class="form-group">
            ...
            <input id="s" name="s" type="search" value="<?php if (is_search()) {
                echo get_search_query();
                   ...
                   placeholder="<?php _e('Search', 'roots-immo'); ?> <?php bloginfo('name'); ?>" autocomplete="off">
